...
export function ImportExportModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-[var(--c-panel)] border border-[var(--c-border)] rounded-xl shadow-2xl w-[580px] max-h-[75vh] flex flex-col"
            ...
            {(['export', 'import'] as const).map((tab_) => (
              <button key={tab_} onClick={() => { setTab(tab_); setError(null) }}
                className={`px-3 py-1 rounded-lg text-sm transition-colors ${
                  tab === tab_ ? 'bg-blue-600 text-white' : 'text-[var(--c-muted)] hover:text-[var(--c-text)]'
                }`}>
                {tab_ === 'export' ? t.exportTab : t.importTab}
              </button>
            ))}
          </div>
          <button onClick={onClose} className="text-[var(--c-subtle)] hover:text-[var(--c-text)] p-1 rounded transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        {tab === 'export' ? (
          <>
            <textarea readOnly value={json}
              className="flex-1 bg-[var(--c-panel-alt)] text-[var(--c-text)] text-xs font-mono p-4 resize-none outline-none min-h-[300px]" />
            <div className="px-5 py-3 border-t border-[var(--c-border)] flex items-center justify-between">
              <span className="text-xs text-[var(--c-subtle)]">{shapes.length} {t.shapes}</span>
          ...
          <>
            <textarea value={importText} onChange={(e) => { setImportText(e.target.value); setError(null) }}
              ...
              className="flex-1 bg-[var(--c-panel-alt)] text-[var(--c-text)] text-xs font-mono p-4 resize-none outline-none min-h-[300px] placeholder-[var(--c-subtle)]" />
            {error && <div className="px-5 py-2 text-red-400 text-xs border-t border-[var(--c-border)]">{error}</div>}
            <div className="px-5 py-3 border-t border-[var(--c-border)] flex items-center justify-between">
              <span className="text-xs text-[var(--c-subtle)]">{t.hint}</span>
